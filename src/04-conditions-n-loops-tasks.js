...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 1; i < pathes.length; i += 1) {
    for (let j = 0; j < path.length; j += 1) {
      if (path[j] === pathes[i][j]) count += 1;
      else break;
    }
    counts.push(count);
    count = 0;
  }
  const length = Math.min(...counts);
  const finish = path.slice(0, length);
  return finish.slice(0, finish.split('').lastIndexOf('/') + 1);
}
...
function getMatrixProduct(m1, m2) {
  ...
  const m3 = Array.from({ length: rowsM1 }, () => Array(colsM2).fill(0));
...
function evaluateTicTacToePosition(position) {
  ...
  const transMatrix = transposed(position);
  for (let i = 0; i < transMatrix.length; i += 1) {
    if (
      transMatrix[i][0] &&
      transMatrix[i][0] === transMatrix[i][1] &&
      transMatrix[i][1] === transMatrix[i][2]
    ) {
      return transMatrix[i][0];
    }
  }
